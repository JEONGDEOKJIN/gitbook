# 자바스크립트에서 스마트 컨트랙트 배포  및 Counter 증감(상태변수 증감)
...
2. 컨트랙트 코드 작성
3. 컨트랙트 코드 '컴파일'
4. 스마트 컨트랙트 배포 (트랜잭션 생성)
5. 컨트랙트 배포 후 CA 받기
6. 배포한 스마트 컨트랙트 실행 위해 abi 변수 만들기
7. getValue 로 상태변수 '조회' 하기 | ⭐⭐목표는 상태 변수로서 저장소에 저장되어 있는 value 값 가져오기
8. setValue 로 상태변수 '변경' 하기 | ⭐⭐목표는 상태 변수로서 저장소에 저장되어 있는 value 변경해보기
